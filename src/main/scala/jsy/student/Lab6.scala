...
object Lab6 extends jsy.util.JsyApplication with Lab6Like {
  ...
  def test(re: RegExpr, chars: List[Char])(sc: List[Char] => Boolean): Boolean = (re, chars) match {
    ...
    case (RSingle(c1), c2 :: t) => if(c1 == c2) sc(t) else false    //test whole string for single char
    case (RConcat(re1, re2), _) => test(re1, chars){charsp => test(re2, charsp)(sc)}   //test both are ture
    case (RUnion(re1, re2), _) => test(re1, chars)(sc) || test(re2, chars)(sc)   //test either are true
    case (RStar(re1), _) => sc(chars) || test(re1, chars){charsp => charsp != chars && test(RStar(re1),  charsp)(sc)} //0 or more
      //have to test length to make sure it's eliminating element. if not they it fails

    /* Extended Operators */
    case (RAnyChar, Nil) => false
    case (RAnyChar, _ :: t) => sc(t)
    case (RPlus(re1), _) => test(RConcat(re1, RStar(re1)), chars)(sc)   //one or more
    case (ROption(re1), _) => sc(chars) || test(re1, chars)(sc)     //{charsp => test(re1, charsp)(sc)} // 0 or 1

    /***** Extra Credit Cases *****/
    case (RIntersect(re1, re2), _) => test(re1, chars)(sc) && test(re2, chars)(sc)
    case (RNeg(re1), _) => !test(re1, chars)(sc)    //is this right?
  }
